...
function getFullAccessPlanContext() {
  ...
  return {
    ...
    permission_keys: [...new Set(Object.values(rolePermissaoMap).flat().concat('dashboard.access'))],
    full_access: true,
    trial: {
      active: false,
      ends_at: null,
...
function buildPlanContext(tenant: any, now = new Date()) {
  ...
  const permissionKeys = [...new Set(
    moduleRoleKeys.flatMap((roleKey) => rolePermissaoMap[roleKey] || []).concat('dashboard.access'),
  )];
