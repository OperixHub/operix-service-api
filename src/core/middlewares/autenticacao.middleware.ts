...
async function resolveTenantId(decoded: any, user?: { tenant_id?: number | null }) {
  ...
  for (const groupName of groups) {
    const tenant = await LocatarioRepository.findByName(String(groupName));
    if (tenant?.id) {
      return tenant.id;
    }
  }
...
async function provisionUser(decoded: any): Promise<any> {
  ...
  const byKcId = await UsuariosRepository.findByKeycloakId(keycloakId);
  if (byKcId) {
    return byKcId;
  }

  const byEmail = await UsuariosRepository.findByEmail(email);
  if (byEmail) {
...
export default class AutenticacaoMiddleware {
  ...
  static async autenticarToken(req: Request, res: Response, next: NextFunction) {
    const isPublicAsset = /(^\/favicon\.ico$)|(\.(ico|png|jpg|jpeg|gif|svg|webp|css|js|map)$)/i.test(req.path);
    if (isPublicAsset) {
      return next();
    }
