...
class UsersRepository {
  ...
  static async create(user: UserModel) {
    const connect = await connection.connect();
    const result = await connect.query(
      'INSERT INTO users (keycloak_id, tenant_id, name, username, email, password, root, admin) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *',
      [user.keycloak_id, user.tenant_id, user.name, user.username, user.email, user.password, user.root, user.admin],
    );
    connect.release();
