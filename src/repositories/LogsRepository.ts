...
class LogsRepository {
  static async insertLog(data: any) {
    ...
    // UUID v4 as pk since we defined it as UUIDV4 in migration
    const id = uuidv4();
    ...
    try {
      ...
      console.error("[LogsRepository] Fallback Error to persist log", e);
    } finally {
      connect.release();
    }
  }
  ...
  static async getPaginatedLogs(tenant_id: any, page = 1, limit = 50) {
    ...
    try {
      ...
      // If tenant_id provided restrict, otherwise pull all (if root).
      if (tenant_id) {
