...
export default class UsersRepository {
  static async getAll() {
    ...
    const users = await connect.query(
      "SELECT id, username, email, root, admin, signature FROM users"
    );
    connect.release();
    return users.rows;
  }

  static async getById(user: any) {
    const connect = await connection.connect();
    const users = await connect.query(
      `SELECT id, username, root, admin FROM users WHERE id = ${user.id}`
    );
    connect.release();
    return users.rows;
  }

  static async getSignature(user: any) {
    const connect = await connection.connect();
    const users = await connect.query(
